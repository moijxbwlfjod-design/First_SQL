...
select p.first_name, a.appointment_date, d.first_name from appointments as a inner join doctors as d on d.doctor_id = a.doctor_id inner join patients as p on a.patient_id = p.patient_id;
...
select gender, count(gender) as genderNumber from patients  group by gender having genderNumber >= 2;
create view myView as select * from admissions where curdate() between admission_date and discharge_date;